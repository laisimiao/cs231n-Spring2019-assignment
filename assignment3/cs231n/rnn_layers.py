...
def rnn_step_backward(dnext_h, cache):
    ...
    # As (tanh)' = 1 - (tanh)^2
    dinter = (1 - next_h ** 2) * dnext_h
    db = np.sum(dinter, axis=0)
    dx = np.dot(dinter, Wx.T)
    dWx = np.dot(x.T, dinter)
    dprev_h = np.dot(dinter, Wh.T)
    dWh = np.dot(prev_h.T, dinter)
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    next_h_per_step, cache_per_step = rnn_step_forward(x[:,0,:], h0, Wx, Wh, b)
    h[:,0,:] = next_h_per_step
    cache.append(cache_per_step)

    for i in range(1,T):
        next_h_per_step, cache_per_step = rnn_step_forward(x[:,i,:], next_h_per_step, Wx, Wh, b)
        h[:,i,:] = next_h_per_step
        cache.append(cache_per_step)
...
def rnn_backward(dh, cache):
    ...
    N, D = cache[0][0].shape
    dx = np.zeros((N, T, D))
    dprev_step_h = np.zeros((N, H))
    dWx = np.zeros((D, H))
    dWh = np.zeros((H, H))
    db = np.zeros((H))

    for t in range(T-1,-1,-1):
        dx[:,t,:], dprev_step_h, dWx_step, dWh_step, db_step = rnn_step_backward(dh[:,t,:] + dprev_step_h, cache[t])
        dWx += dWx_step
        dWh += dWh_step
        db += db_step
